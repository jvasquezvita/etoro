...
st.set_page_config(page_title="WAR ROOM - 10X LEVERAGE", layout="wide", initial_sidebar_state="collapsed")
...
def cargar_tickers_permanentes():
    if os.path.exists(ARCHIVO_TICKERS):
        with open(ARCHIVO_TICKERS, "r") as f: return [l.strip().upper() for l in f.read().splitlines() if l.strip()]
    return TICKERS_DEFECTO

def guardar_tickers_permanentes(lista_tickers):
    with open(ARCHIVO_TICKERS, "w") as f:
        for t in lista_tickers: f.write(f"{t}\n")

if "tickers" not in st.session_state: st.session_state.tickers = cargar_tickers_permanentes()
if "seleccionado" not in st.session_state: st.session_state.seleccionado = st.session_state.tickers[0] if st.session_state.tickers else None
...
def obtener_datos_ticker(ticker_symbol):
    try:
        ...
        # ⚡ REGRESO A 15 MINUTOS: Máxima precisión para operaciones a x10
        ...
        # PARÁMETROS ESTRICTOS PARA X10
        if rsi_actual <= 30:
            bg_color, border_color, senal = "#0f3d1b", "#00ff41", "🟢 LONG x10"
            # Lógica LONG: Busco subida
            target = precio_actual * 1.012  # +1.2% activo = +12% ROI
            stop = precio_actual * 0.996    # -0.4% activo = -4% ROI
        elif rsi_actual >= 70:
            bg_color, border_color, senal = "#4d1111", "#ff3333", "🔴 SHORT x10"
            # Lógica SHORT: Busco caída
            target = precio_actual * 0.988  # -1.2% activo = +12% ROI
            stop = precio_actual * 1.004    # +0.4% activo = -4% ROI
        else:
...
with col_longs:
    ...
    for t, data in dict(sorted(activos_long.items(), key=lambda item: item[1]["rsi"])).items():
        ...
        with st.container(border=True):
            ...
            """, unsafe_allow_html=True)
            if st.button(f"Ver {t}", key=f"long_{t}", use_container_width=True): st.session_state.seleccionado, st.rerun() = t, True
...
with col_shorts:
    ...
    for t, data in dict(sorted(activos_short.items(), key=lambda item: item[1]["rsi"], reverse=True)).items():
        ...
        with st.container(border=True):
            ...
            """, unsafe_allow_html=True)
            if st.button(f"Ver {t}", key=f"short_{t}", use_container_width=True): st.session_state.seleccionado, st.rerun() = t, True

with col_orden:
    st.markdown("### 🛠️ GESTIÓN DE RIESGO X10")
    # (El código de agregar/eliminar tickers sigue igual que antes, lo resumo para enfocarnos en la salida)
    ...
    if target_ticker in datos_completos:
        ...
        with st.container(border=True):
            ...
            st.error(f"🛑 **STOP LOSS**: ${main['stop_loss']:.2f} (Riesgo real: **-4%** del capital)")
            st.success(f"🏆 **TAKE PROFIT**: ${main['target']:.2f} (Ganancia real: **+12%** del capital)")
            
            st.caption("Nota: Cálculos basados en apalancamiento estricto 10x. No mover el Stop Loss en contra.")

time.sleep(20) # Refresco cada 20 seg para scalping
st.rerun()
